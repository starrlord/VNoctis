 ...
 * Follows the same modal pattern as CreateUserModal
 * (backdrop blur, body scroll lock, escape-to-close, focus trap).
 *
 * @param {{ userId: string, username: string, onClose: () => void, onReset: (userId: string, password: string) => Promise<void> }} props
 */
export default function ResetPasswordModal({ userId, username, onClose, onReset }) {
    ...
    >
      ...
      >
        ...
        <p className="px-6 text-sm text-gray-500 dark:text-gray-400">
          Set a new password for <strong className="text-gray-700 dark:text-gray-200">{username}</strong>
        </p>
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 pt-4 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 disabled:text-blue-300 text-white rounded-lg text-sm font-semibold transition-colors flex items-center gap-2"
            >
              {saving ? (
                <>
